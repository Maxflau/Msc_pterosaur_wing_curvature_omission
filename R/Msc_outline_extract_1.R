# Extract wing outlines from specimen images and save them in NTS format.
extract_simple_outlines_fuzzy <- function(image_folder,
                                          specimen_names,
                                          output_file = "Pterosaur_Outlines",
                                          threshold = 50,
                                          num_points = 100) {
  # List the image files twice: once as names and once as full paths.
  ...
  # Match each specimen name to the most likely image file.
  ...
  # Prepare containers for the extracted outlines and the success log.
  all_outlines <- list()
  ...
  # Work through the specimen list one by one.
  for (i in seq_along(specimen_names)) {
    ...
    # Read the image, keep the wing pixels, then trace a simple outline.
    tryCatch({
      ...
      # Resample the outline so every specimen has the same number of points.
  ...
  # Save the extracted outlines in the text format used by later scripts.
  ...
  # Save a simple log showing which specimens worked and which failed.
...
# Run the extraction step only when the raw image folder is available.
